...
export class AddDateColumnTeam1700000000004 implements MigrationInterface {
  name = 'AddDateColumnTeam1700000000004';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      await queryRunner.createIndex(
        'team',
        new TableIndex({
          name: 'IDX_TEAM_DELETED_AT',
          columnNames: ['deleted_at'],
        }),
      );
      console.log('✅ IDX_TEAM_DELETED_AT index created successfully');
    } catch (error) {
      console.log(
        'ℹ️ IDX_TEAM_DELETED_AT index already exists or creation failed:',
        error.message,
      );
    }
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Drop index first
    try {
      await queryRunner.dropIndex('team', 'IDX_TEAM_DELETED_AT');
      console.log('✅ IDX_TEAM_DELETED_AT index dropped successfully');
    } catch (error) {
      console.log(
        'ℹ️ IDX_TEAM_DELETED_AT index does not exist or drop failed:',
        error.message,
      );
    }
    ...
    try {
      await queryRunner.dropColumn('team', 'deleted_at');
    ...
    try {
      await queryRunner.dropColumn('team', 'updated_at');
    ...
    try {
      await queryRunner.dropColumn('team', 'created_at');
